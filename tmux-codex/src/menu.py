...
class SessionMenu:
    ...
    def __init__(self, tmux: "TmuxClient"):
        ...
        self._migrate_legacy_codex_files()
    ...
    def _migrate_legacy_codex_files(self):
        """One-time migration for runner prefs and session tags into .codex."""
        legacy_root = self._dev_path / ".opencode"
        codex_root = self._dev_path / ".codex"

        migration_pairs = [
            (legacy_root / "config" / "runner-prefs.json", codex_root / "config" / "runner-prefs.json"),
            (legacy_root / "session-tags.json", codex_root / "session-tags.json"),
        ]

        for src, dest in migration_pairs:
    ...
    def _get_runner_prefs_path(self) -> Path:
        ...
        return self._dev_path / ".codex" / "config" / "runner-prefs.json"
    ...
    def _get_tags_path(self) -> Path:
        ...
        return self._dev_path / ".codex" / "session-tags.json"
    ...
    def _record_runner_end_times(self, runner_names: list[str]):
        ...
        log_file = self._dev_path / ".codex" / "logs" / "runners" / "runners.log"
    ...
    def _start_runner_session(self) -> str | None:
        ...
        runctl_path = Path(os.environ.get("TMUX_CLI_HOME", "/Users/jian/Dev/tmux-codex")) / "bin" / "runctl"
