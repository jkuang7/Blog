...
class SessionMenuRunTests(unittest.TestCase):
    ...
    def test_migrates_legacy_prefs_and_tags_into_codex_namespace(self):
        with tempfile.TemporaryDirectory() as tmp:
            dev = Path(tmp)
            (dev / ".opencode" / "config").mkdir(parents=True)
            (dev / ".opencode" / "config" / "runner-prefs.json").write_text('{"enabled_projects":["Blog"]}\n')
            (dev / ".opencode" / "session-tags.json").write_text('{"codex-1":"Session"}\n')
